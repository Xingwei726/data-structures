...
function makeCSV(jsonFile,writeFile){
    ...
    fs.writeFileSync('data/AAAll.csv', "LocationTitle,AddressStreetInfo,City,State,Zipcode,Details,Latitude,Longitude,Zone,GroupTitle,Day,Start,End,Meeting_Type\n");
        ...
        if (meetings.hasOwnProperty(locationName)) {
            ...
            for (var meetingName in meetings[locationName]['meetings']){
                ...
                for (let i = 0; i < meetings[locationName]['meetings'][meetingName].length; i++) { 
                    ...
                    fs.appendFileSync('data/AAAll.csv', saveString + '\n');
                }
            }
        }
    }
